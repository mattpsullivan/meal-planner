...
function extractMealsFromDay(day: MealplanDay, recipeSlugMap: Map<string, string>): ExportMeal[] {
  const meals: ExportMeal[] = [];
  let order = 0;
...
export function mealplanToExportWeek(
  mealplan: Mealplan,
  recipeSlugMap: Map<string, string>
): ExportWeek {
  ...
  const prepSteps: ExportPrepStep[] = (mealplan.prep_timeline ?? []).map((step, idx) => {
  ...
  // Convert days
  const days: ExportDay[] = mealplan.days.map((day) => ({
    day_number: day.day,
    date: day.date ?? '',
...
export function getReferencedRecipePaths(mealplan: Mealplan): Set<string> {
  ...
  for (const day of mealplan.days) {
    ...
    if (meals.breakfast?.recipe) paths.add(meals.breakfast.recipe);
    if (meals.lunch?.recipe) paths.add(meals.lunch.recipe);
    if (meals.dinner?.recipe) paths.add(meals.dinner.recipe);
    if (meals.snacks) {
      for (const snack of meals.snacks) {
        if (snack.recipe) paths.add(snack.recipe);
      }
    }
  }
  ...
  if (mealplan.prep_timeline) {
    for (const step of mealplan.prep_timeline) {
      if (step.recipe) paths.add(step.recipe);
    }
  }
