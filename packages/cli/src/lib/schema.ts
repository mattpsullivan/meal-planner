...
import type { MealplanExport } from './types.js';
...
export function validateMealplanExport(data: unknown): ValidationResult {
  ...
  if (valid) {
    const export_ = data as unknown as MealplanExport;
    ...
    for (const week of export_.weeks) {
      for (const component of week.components) {
        if (!recipeIds.has(component.recipe_id)) {
          errors.push(
            `Week ${String(week.week_number)}: Component references missing recipe "${component.recipe_id}"`
          );
        }
      }
      for (const step of week.prep_steps) {
        if (step.recipe_id && !recipeIds.has(step.recipe_id)) {
          warnings.push(
            `Week ${String(week.week_number)}: Prep step references missing recipe "${step.recipe_id}"`
          );
        }
      }
      for (const day of week.days) {
        for (const meal of day.meals) {
          if (meal.recipe_id && !recipeIds.has(meal.recipe_id)) {
            warnings.push(
              `Week ${String(week.week_number)}, Day ${String(day.day_number)}: Meal references missing recipe "${meal.recipe_id}"`
            );
          }
        }
      }
