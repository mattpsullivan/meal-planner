...
function extractInstructions(sections, ingredients, cookware = [], timers = []) {
    ...
    for (const section of sections) {
        ...
        for (const step of section.content) {
            if (step.type !== 'step' || !step.value?.items)
                continue;
...
// =========== Recipe Parsing ===========
export function parseRecipeFile(filePath, parser, recipesDir, recipeMap) {
    ...
    if (typeof rawTags === 'string') {
        ...
        tags = rawTags.map((t) => t.trim().toLowerCase());
    }
    // Custom metadata
...
export function findCookFiles(dir) {
    const files = [];
